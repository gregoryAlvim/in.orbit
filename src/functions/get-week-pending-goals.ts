...
export async function getWeekPendingGoals() {
  ...
  const goalsCompletionsCounts = await db.$with('goals_completions_counts').as(
    db
      .select({
  ...
  const pendingGoals = await db
    ...
    .select({
      ...
      completionCount:
        sql`COALESCE(${goalsCompletionsCounts.completionCount}, 0)`.mapWith(
          Number
        ),
    })
